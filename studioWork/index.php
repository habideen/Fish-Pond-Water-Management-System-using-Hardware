...
//get admin code
$controlKey = $controlPrefix = $controlSuffix = '';
if ( isset($_GET['key']) ) {
	$controlKey = strtolower( $_GET['key'] );
	$controlPrefix = substr($controlKey, 0);

	//redirect to home page if login phrase is invalid
	if ($controlPrefix != 'control_credit'
		&& $controlPrefix != 'control_cro'
		&& $controlPrefix != 'control_admin'
		&& $controlPrefix != 'control_blog')
		header('Location: home');
}
else
	header('Location: home');
